...
async function countFormControls(page: Page) {
  return page.evaluate(() => {
    ...
    const prefilled = controls.filter((el) => {
      ...
      if (el instanceof HTMLSelectElement) return (el.selectedIndex > 0 || (el.value ?? '').trim().length > 0);
...
test.describe('Product authoring UX audit', () => {
  ...
  test('AI Studio — unauthenticated gate', async ({ page }) => {
    ...
    notes.push(onAuth ? 'Redirects to /auth when unauthenticated (expected).' : `Unexpected URL: ${finalUrl}`);
  ...
  test('Central — unauthenticated gate', async ({ page }) => {
    ...
    notes.push(onLogin ? 'Redirects to /login when unauthenticated (expected).' : `Unexpected URL: ${finalUrl}`);
  ...
  test('AI Studio — authenticated dry-run (optional)', async ({ page }) => {
    ...
    test.skip(!email || !password, 'Set TEST_STUDIO_EMAIL and TEST_STUDIO_PASSWORD for live authoring metrics');
    ...
    const classTrigger = page.locator('[id*="product_class"], button:has-text("Product class")').first();
    if (await classTrigger.isVisible().catch(() => false)) {
      ...
      const classSelect = page.locator('select').filter({ hasText: /bulk|ready|gift/i }).first();
  ...
  test('Central — authenticated dry-run (optional)', async ({ page }) => {
    ...
    test.skip(!email || !password, 'Set TEST_CENTRAL_EMAIL and TEST_CENTRAL_PASSWORD for live authoring metrics');
    ...
    await page.getByText(/identity|product name/i).first().waitFor({ timeout: 30_000 });
    const timeToFormReadyMs = Date.now() - t0;
    await snap(page, 'central', '03-new-product-panel', screenshots);

    const nameInput = page.locator('input').filter({ has: page.locator('xpath=..//label[contains(.,"Product Name") or contains(.,"Name")]') }).first();
