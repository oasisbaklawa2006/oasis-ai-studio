-- PR-06C1b: Wave 4A-0 packaging scalar approval mapping
-- Extends approve_catalogue_draft_internal PRODUCT branch only.
-- Tag + alias branches unchanged from PR06C1a.
-- Staging only until verified. Do NOT apply to production without sign-off.
...
BEGIN
  ...
  -- -------------------------------------------------------------------------
  -- catalogue_product_drafts branch (PR06C1b Wave 4A-0 packaging scalar mapping)
  -- -------------------------------------------------------------------------
  IF p_draft_table = 'catalogue_product_drafts' THEN
    IF v_operation = 'create' OR v_target_record_id IS NULL THEN
      ...
      VALUES (
        ...
        true,
        false,
        -- Wave 4A-0 (PR06C1b): pack_size prefers authority pack_size, not primary_pack_type label
        coalesce(
          nullif(v_payload #>> '{packing,pack_size}', ''),
          nullif(v_payload #>> '{packing,pack_preview}', ''),
          nullif(v_payload #>> '{packing,primary_pack_type}', '')
        ),
        -- Wave 4A-0: packaging_scalars -> products (create path; absent keys stay NULL)
        nullif(v_payload #>> '{packing,packaging_scalars,grams_per_piece}', '')::numeric,
        nullif(v_payload #>> '{packing,packaging_scalars,pcs_per_kg}', '')::numeric,
        nullif(v_payload #>> '{packing,packaging_scalars,primary_pack_weight_kg}', '')::numeric,
        nullif(v_payload #>> '{packing,packaging_scalars,pcs_per_primary_pack}', '')::numeric,
        coalesce(
          nullif(v_payload #>> '{packing,packaging_scalars,carton_type}', ''),
          nullif(v_payload #>> '{packing,carton_type}', '')
        ),
        nullif(v_payload #>> '{packing,packaging_scalars,pcs_per_master_carton}', '')::numeric,
        nullif(v_payload #>> '{packing,packaging_scalars,packs_per_master_carton}', '')::numeric,
        nullif(v_payload #>> '{packing,packaging_scalars,packs_per_carton}', '')::numeric,
    ...
    ELSE
      ...
      SET
        ...
        -- Wave 4A-0 (PR06C1b): only promote explicit pack_size; never overwrite with primary_pack_type
        pack_size = coalesce(
          nullif(v_payload #>> '{packing,pack_size}', ''),
          pack_size
        ),
        -- Wave 4A-0: packaging_scalars coalesce — never null-out existing master values
        grams_per_piece = coalesce(
          nullif(v_payload #>> '{packing,packaging_scalars,grams_per_piece}', '')::numeric,
          grams_per_piece
        ),
        pcs_per_kg = coalesce(
        ...
        primary_pack_weight_kg = coalesce(
          nullif(v_payload #>> '{packing,packaging_scalars,primary_pack_weight_kg}', '')::numeric,
          primary_pack_weight_kg
        ),
        pcs_per_primary_pack = coalesce(
          nullif(v_payload #>> '{packing,packaging_scalars,pcs_per_primary_pack}', '')::numeric,
          pcs_per_primary_pack
        ),
        carton_type = coalesce(
          nullif(v_payload #>> '{packing,packaging_scalars,carton_type}', ''),
          nullif(v_payload #>> '{packing,carton_type}', ''),
          carton_type
        ),
        pcs_per_master_carton = coalesce(
          nullif(v_payload #>> '{packing,packaging_scalars,pcs_per_master_carton}', '')::numeric,
          pcs_per_master_carton
        ),
        packs_per_master_carton = coalesce(
          nullif(v_payload #>> '{packing,packaging_scalars,packs_per_master_carton}', '')::numeric,
          packs_per_master_carton
        ),
        packs_per_carton = coalesce(
          nullif(v_payload #>> '{packing,packaging_scalars,packs_per_carton}', '')::numeric,
          packs_per_carton
        ),
