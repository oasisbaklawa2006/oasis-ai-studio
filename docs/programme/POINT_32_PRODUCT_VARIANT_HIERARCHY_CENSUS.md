...
## Status update — 2026-09-19 (APPVERSE-AI-FINAL-01)

The "CORE BLOCKED" rows below are historical — they described the gap at the time of the
original census. Core PR #310 (migration `20260915240000_point32_product_variant_authority.sql`)
has since landed on `oasis-supabase-core` `main`, deploying `products.basis_product_id` and the
`product_variants` table (deterministic `variant_key` uniqueness per basis product, fail-closed
identity-lock triggers, and RLS restricting writes to `public.is_admin()`). AI Studio's consumer
contract (`productVariantHierarchyCanonical.ts`) has been reconciled to bind to that live
authority: `basis_product_id`/`variant_key` are read from the form as real columns (never
inferred), `validateVariantHierarchyMutation` now performs structural validation instead of
universally blocking, and the Full Editor identity tab (`ProductVariantHierarchyPanel`) surfaces
and can set them, with Core's own RLS remaining the authoritative write gate — never bypassed
client-side. Generated Supabase types (`src/integrations/supabase/types.ts`) were hand-extended
for `product_variants` and `products.basis_product_id` pending a live `generate_typescript_types`
regen, the same residual pattern already flagged for Central's Point27 pricing/moq draft types.
