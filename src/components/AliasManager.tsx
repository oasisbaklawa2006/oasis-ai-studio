...
import { governedAliasSeedsFromSource } from "@/features/governedMultilingual";
...
export function AliasManager({ productId, productName, id: sectionId, onAliasesChange }: Props) {
  ...
  const [, setTermTypeVersion] = useState(0);
  ...
  const load = useCallback(async () => {
    ...
    onAliasesChange?.();
  }, [onAliasesChange, productId, productName]);
  ...
  const itemsWithTermType = items.map((item) => ({
    item,
    termType: resolveTermType(productId, item),
  }));
  const tabItems = itemsWithTermType.filter(({ termType }) => termType === activeTermType);
  ...
  const addOne = async () => {
    ...
    setSubmitting(true);
    try {
      const row: AliasRowInput = {
        ...draft,
        alias: trimmed,
        source: "manual",
        term_type: activeTermType,
      };
  ...
  const generate = async () => {
    ...
    try {
      ...
      if (submitted > 0) {
        toast.success(`Submitted ${submitted} draft${submitted === 1 ? "" : "s"} for approval.`);
      }
    } finally {
      setSubmitting(false);
    }
  ...
  const remove = async (item: any) => {
    ...
    try {
      ...
      toast.success(
        "Delete request submitted for approval. This alias stays visible until review.",
      );
    } finally {
      setSubmitting(false);
    }
    ...
    <div id={sectionId} className="card-elevated p-6 space-y-4">
      <div className="flex items-start justify-between gap-2">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Typed search and channel vocabulary — misspellings, customer phrases, WhatsApp keywords,
            regional names.
          </p>
        ...
        {canMutate && (
          <div className="flex gap-2">
            ...
            <Button
              ...
              variant="ghost"
              onClick={copyAll}
              disabled={!tabItems.length || submitting}
            >
      ...
      <div className="rounded-md border bg-muted/20 p-3 space-y-1">
        ...
        <p className="text-[11px] text-muted-foreground">
          Sourced from products.name — edit on the Identity tab.
        </p>
      </div>

      <Tabs
        value={activeTermType}
        onValueChange={(v) => setActiveTermType(v as ProductLanguageTermType)}
      >
          ...
          <TabsContent key={termType} value={termType} className="mt-4 space-y-3">
            {termType === "whatsapp_keyword" && (
              <p className="text-xs text-warning border border-warning/30 bg-warning/5 rounded-md px-2 py-1.5">
                WhatsApp keywords affect order matching and require reviewer approval before use in
                chat flows.
              </p>
            )}
              ...
              <div className="grid sm:grid-cols-[1fr_120px_auto] gap-2 items-end">
                <div>
                  ...
                  <Input
                    ...
                    placeholder={
                      termType === "whatsapp_keyword" ? "e.g. need cashew kitta" : "e.g. Kaju Kitta"
                    }
                    disabled={submitting}
                  />
                </div>
                  ...
                  >
                    ...
                    {writeMode === "direct" && canMutate && (
                      <select
                        ...
                        value={itemTermType}
                        onChange={(e) =>
                          changeItemTermType(item.id, e.target.value as ProductLanguageTermType)
                        }
