  ...
  return <div className="card-elevated p-6 space-y-4">
    ...
    <div className="grid gap-3">
      {rows.map((r) => <div key={r.id} className="rounded-xl bg-white shadow-sm border border-teal-100 p-4 space-y-3">
        <div className="flex items-start justify-between"><div className="font-semibold text-base">{(r.channel || "channel").replace(/_/g, " ")}</div><div className="flex gap-2"><Button size="sm" variant="outline" onClick={() => setShowAdvanced((s) => ({ ...s, [r.id]: !s[r.id] }))}><ChevronDown className="h-3 w-3 mr-1" />Edit</Button><Button size="sm" variant="ghost" onClick={() => update(r.id, { channel: `${r.channel}_copy` })}><Copy className="h-3 w-3 mr-1" />Duplicate</Button><Button size="sm" variant="ghost" onClick={() => update(r.id, { moq_applicable: false })}><Power className="h-3 w-3 mr-1" />Disable</Button></div></div>
        ...
        {showAdvanced[r.id] && <div className="grid sm:grid-cols-2 gap-2 rounded-lg border p-3">
