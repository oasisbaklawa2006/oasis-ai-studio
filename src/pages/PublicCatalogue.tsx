...
import { Download, MessageCircle, QrCode, Mail } from "lucide-react";
...
const PublicCatalogue = () => {
  ...
  useEffect(() => {
    (async () => {
      const { data } = await supabase.from("catalogues").select("*").eq("public_slug", slug).eq("is_published", true).maybeSingle();
  ...
  if (!cat) return <div className="min-h-screen flex flex-col items-center justify-center text-center p-8">
    <div className="font-display text-3xl mb-2">Catalogue not found</div>
    <p className="text-muted-foreground">This catalogue link may be unpublished or expired.</p>
  </div>;
      ...
      <section className="max-w-5xl mx-auto px-6 pb-16">
        <div className="card-elevated p-8 text-center">
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            ...
            <Button variant="ghost" disabled><Download className="h-4 w-4 mr-1" />Download PDF</Button>
            <Button variant="ghost" disabled><QrCode className="h-4 w-4 mr-1" />QR Share</Button>
          </div>
        </div>
      </section>

      <footer className="border-t py-8 text-center text-xs text-muted-foreground">
        © Oasis Baklawa · Catalogue powered by Oasis AI Studio
      </footer>
    </div>
  );
};
