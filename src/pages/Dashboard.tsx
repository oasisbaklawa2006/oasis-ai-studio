import { BookOpen, Gift, Plus, Sparkles, Tag, Upload, Wand2, Zap } from "lucide-react";
    ...
    (async () => {
      const [p, m, l] = await Promise.all([
        ...
        supabase
          ...
          .neq("label_status", "approved"),
      ]);
      setStats({
        products: p.count ?? 0,
        catalogues: 0,
        media: m.count ?? 0,
        pendingLabels: l.count ?? 0,
      });
      setRecent([]);
    })();
  }, []);
  ...
  const ai = [
    {
      ...
      desc: "Upgrade raw iPhone photos to luxury-grade catalogue shots.",
      status: "Planned",
    },
    {
      title: "Product Identification",
      desc: "AI recognizes product from photo and links to master.",
      status: "Planned",
    },
    {
      title: "Video / Reel Generation",
      desc: "Auto reels for WhatsApp & social.",
      status: "Planned",
    },
    {
      title: "Compliance Auto-Labeling",
      desc: "Draft FSSAI-style label data; human approves.",
      status: "Planned",
    },
  ];
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="Your premium catalogue intelligence at a glance."
      />

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
        <StatCard label="Products" value={stats.products} hint="In master" />
        <StatCard label="Active Catalogues" value="On hold" hint="Backend not deployed" />
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-3 mb-10">
        {[
          ...
          <Button asChild variant="outline" key={a.to} className="h-auto py-4 justify-start">
            <Link to={a.to}>
              <a.icon className="h-4 w-4 mr-2 text-accent" />
              {a.label}
            </Link>
          </Button>
        ))}
      ...
      <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="card-elevated divide-y">
            {recent.length === 0 && (
              <div className="p-6 text-sm text-muted-foreground">
                Catalogue persistence is safely on hold until its governed backend is deployed.
              </div>
            )}
            {recent.map((c) => (
              <div key={c.id} className="p-4 flex items-center justify-between">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground capitalize">
                    {(c.status ?? (c.is_published ? "published" : "draft")).replace(/_/g, " ")}
                    {c.public_slug ? ` · /c/${c.public_slug}` : ""}
                  </div>
                </div>
                <Button asChild variant="ghost" size="sm">
                  <Link to={`/catalogues/${c.id}`}>Open</Link>
                </Button>
              </div>
            ))}
          </div>
        ...
        <div>
          <h2 className="font-display text-2xl mb-3 flex items-center gap-2">
            <Sparkles className="h-5 w-5 text-accent" /> AI Studio
          </h2>
              ...
              <div key={m.title} className="card-elevated p-4">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-2">
                    <Wand2 className="h-4 w-4 text-accent mt-0.5" />
                    <div className="font-medium text-sm">{m.title}</div>
                  </div>
                  <span className="badge-soft bg-accent-soft text-accent-foreground">
                    {m.status}
                  </span>
                </div>
