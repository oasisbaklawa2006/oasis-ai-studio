import { CheckCircle2, Circle, FileText, Trash2, Upload } from "lucide-react";
...
const SECTIONS: Section[] = [
  {
    ...
    items: [
      ...
      {
        id: "A2",
        label: "Confirm Owner role in sidebar footer",
        hint: "Sidebar shows role under email.",
      },
  ...
  {
    id: "B",
    title: "Product + SKU Test",
    items: [
      { id: "B1", label: "Add product", hint: "Products → New Product." },
      {
        id: "B2",
        label: "Pick division/category/subcategory/packaging",
        hint: "Inside the SKU section.",
      },
  ...
  },
  {
    ...
    items: [
      ...
      {
        id: "C2",
        label: "Generate starter aliases",
        hint: "Try product name 'Kunafa' or 'Cashew Pyramid'.",
      },
      {
        id: "C3",
        label: "Search by misspelling/local name",
        hint: "e.g. 'kataifi', 'kaju pyramid'.",
      },
      {
        id: "C4",
        label: "Confirm 'Matched by alias' shows",
        hint: "In Products and ProductPicker.",
      },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      {
        id: "E2",
        label: "Search products by SKU/name/alias",
        hint: "ProductPicker in detail page.",
      },
  ...
  {
    id: "F",
    title: "Hamper / BOM Test",
    items: [
      { id: "F1", label: "Create hamper", hint: "Hampers → New Hamper." },
      {
        id: "F2",
        label: "Search child product by alias",
        hint: "ProductPicker in component editor.",
      },
  ...
  {
    id: "G",
    title: "Ingredient / Nutrition Test",
    items: [
      { id: "G1", label: "Add ingredients", hint: "Ingredients page." },
      {
        id: "G2",
        label: "Link ingredients to product",
        hint: "Inside product page (future module).",
      },
      { id: "G3", label: "Add nutrition panel", hint: "Per-product nutrition." },
      {
        id: "G4",
        label: "Confirm warning shows for missing data",
        hint: "Label Queue lists warnings.",
      },
    ],
  },
  {
  ...
  {
    id: "I",
    title: "Integration Readiness Test",
    items: [
      ...
      {
        id: "I3",
        label: "Confirm SKU/product_id identity notes",
        hint: "Notes appear under each card.",
      },
    ],
  },
  {
    id: "J",
    title: "Stability & Mobile Test",
    items: [
      {
        id: "J1",
        label: "390px mobile — no horizontal overflow",
        hint: "Open at 390px width and check every page swipes only top↔bottom.",
      },
      {
        id: "J2",
        label: "Tab-switch — no refresh/reset",
        hint: "Open Product Edit → Media tab → switch browser tab/app → return → tab and form state preserved.",
      },
      {
        id: "J3",
        label: "Product image URL upload",
        hint: "Add via direct .jpg/.png URL — image appears.",
      },
      {
        id: "J4",
        label: "Google Drive image URL",
        hint: "Paste a Drive file link — warning shown, image normalized.",
      },
      {
        id: "J5",
        label: "Replace PDF hero image",
        hint: "Product with PDF hero → upload real photo → hero replaced, PDF marked reference-only.",
      },
      {
        id: "J6",
        label: "BOM linked-product cost autofill",
        hint: "Add BOM child product — cost_per_unit and unit auto-filled, conversion note visible.",
      },
      {
        id: "J7",
        label: "Public catalogue photo fallback",
        hint: "Product without real photo shows elegant placeholder, never PDF screenshot.",
      },
    ],
  },
];
...
const Testing = () => {
  ...
  useEffect(() => {
    ...
    try {
      setAttachments(JSON.parse(localStorage.getItem(PDF_KEY) ?? "[]"));
    } catch {
      /* ignore */
    }
  }, []);
  ...
  const toggle = (id: string) =>
    persist({ ...state, [id]: { done: !state[id]?.done, notes: state[id]?.notes ?? "" } });
  const setNote = (id: string, notes: string) =>
    persist({ ...state, [id]: { done: state[id]?.done ?? false, notes } });

  const persistPdfs = (next: Attachment[]) => {
    setAttachments(next);
  ...
  const uploadPdf = async (files: FileList | null) => {
    ...
    try {
      ...
      persistPdfs([
        { name: f.name, url: pub.publicUrl, uploadedAt: new Date().toISOString() },
        ...attachments,
      ]);
  ...
  const removePdf = (url: string) => {
    ...
    persistPdfs(attachments.filter((item) => item.url !== url));
  };
      ...
      <div className="card-elevated p-4 sm:p-5 mb-6 space-y-3">
        ...
        {attachments.length === 0 ? (
          ...
          <ul className="divide-y">
            {attachments.map((a) => (
              <li key={a.url} className="flex items-center gap-2 py-2 text-sm">
                ...
                <Button size="icon" variant="ghost" onClick={() => removePdf(a.url)}>
                ...
                return (
                  <div key={it.id} className="border rounded-lg p-3">
                    <div className="flex items-start gap-3">
                      <button type="button" onClick={() => toggle(it.id)} className="mt-0.5">
                        {isDone ? (
