...
const ProductEdit = () => {
  ...
  const missing = useMemo(() => {
    ...
    if (!isContributorRole) {
      ...
      if (form.private_label_allowed && (!form.private_label_moq || !form.private_label_cost_per_unit)) m.push("Private label MOQ & cost");
    }
    return m;
  ...
  const save = async () => {
    setSubmitError("");
    if (isContributorRole && import.meta.env.DEV) console.log("[ProductEdit] contributor submit clicked", { isNew, id });
    if (isContributorRole && import.meta.env.DEV) console.log("[ProductEdit] validation result", { missing });
    ...
    if (contributor) {
      ...
      if (import.meta.env.DEV) console.log("[ProductEdit] auth user id", authData.user?.id || null);
      ...
      if (import.meta.env.DEV) console.log("[ProductEdit] draft insert object", groupedPayload);
  ...
  return (
    <>
      ...
      {submitError && (<div className="mb-4 rounded-lg border border-destructive/40 bg-destructive/10 p-3 text-sm text-destructive">{submitError}</div>)}
