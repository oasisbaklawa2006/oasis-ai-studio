...
import {
  validateProductAggregate,
  type ProductValidationIssue,
} from "@/features/productAuthority/productAggregateValidation";
...
const Field = ({
  ...
  const inputId = `product-field-${field ?? generatedId.replace(/:/g, "")}`;
  ...
  return (
    <div className="space-y-1" data-product-field={field}>
      <Label className="text-xs" htmlFor={inputId}>{label}</Label>
      {labelledChild}
      {hint && <div id={hintId} className="text-[11px] text-muted-foreground">{hint}</div>}
    </div>
  );
};

type ProductSelectOption = string | { v: string; label: string };
...
const Select = ({ id, value, onChange, options, placeholder, ...aria }: ProductSelectProps) => (
  <select
    id={id}
    {...aria}
...
const ProductEdit = () => {
  ...
  const refreshAggregateRevision = async (productId: string) => {
    const { data, error } = await (supabase as any)
      .from("products")
      .select("updated_at, aggregate_revision")
      .eq("id", productId)
      .single();
    if (error || !data) {
      if (import.meta.env.DEV) {
        console.error("[ProductEdit] aggregate revision refresh failed:", error?.message);
      }
      return;
    }
    setForm((current: Record<string, unknown>) => ({
      ...
      aggregate_revision: data.aggregate_revision,
    }));
  };
  ...
  const availableEditorTabs = useMemo(() => {
    const tabs = [
      { value: "identity", label: "Identity", visible: true },
      { value: "uom", label: "UOM / MOQ", visible: true },
      { value: "media", label: isTestingMediaGovernance() ? "Hero image" : "Media", visible: !isNew },
      { value: "private_label", label: "Private Label", visible: showPrivateLabel },
      { value: "customisation", label: "Customisation", visible: showCustomization },
      { value: "dimensions", label: "Dimensions", visible: showDimensions },
      { value: "frozen", label: "Frozen", visible: showFrozen },
      { value: "bom", label: "BOM", visible: showBom },
      { value: "channels", label: "Business Rules", visible: !isNew },
      { value: "compliance", label: "Compliance", visible: true },
      { value: "ops", label: "Ops Notes", visible: true },
      { value: "product_truth", label: "Product Truth", visible: !isNew },
    ];
    return tabs.filter((item) => item.visible);
  }, [isNew, showBom, showCustomization, showDimensions, showFrozen, showPrivateLabel]);

  useEffect(() => {
    if (availableEditorTabs.some((item) => item.value === tab)) return;
    setTab("identity");
  }, [availableEditorTabs, tab]);
  ...
  const aggregateValidationIssues = useMemo(
    () => validateProductAggregate(form, { contributorMode: isContributorMode }),
    [form, isContributorMode],
  );
  const aggregateBlockingIssues = useMemo(
    () => aggregateValidationIssues.filter((issue) => issue.severity === "error"),
    [aggregateValidationIssues],
  );

  const openValidationIssue = (issue: ProductValidationIssue) => {
    startTransition(() => setTab(issue.tab));
    requestAnimationFrame(() => {
      document.querySelector(`[data-product-field="${issue.field}"]`)?.scrollIntoView({
        behavior: "smooth",
        block: "center",
      });
    });
  ...
  const save = async () => {
    ...
    if (aggregateBlockingIssues.length > 0) {
      const firstIssue = aggregateBlockingIssues[0];
      const message = `Fix data integrity: ${firstIssue.message}`;
      setSubmitError(message);
      toast.error(message);
      openValidationIssue(firstIssue);
      saveAttemptGateRef.current.release();
      return;
    }
    ...
    if (direct) {
      ...
      // Flush the latest keystroke state before the network request rather than relying
      // only on the debounced autosave. A timeout, session expiry or conflict can then be
      // recovered after refresh with the exact same idempotency identity.
      const recoveryWrite = writeProductDraft(
        localStorage,
        draftKey,
        createProductDraftEnvelope(
          draftKey,
      ...
      if (recoveryWrite.ok) {
        ...
        setDraftPersistenceError(recoveryWrite.message);
      }

      // One server-owned transaction writes the product plus the supplied governed
      // pricing/MOQ children. There is intentionally no browser fallback to separate
      // table writes: a failed receipt must never be presented as a partial success.
      const aggregateSave = await saveProductAggregateAtomic({
        idempotencyKey: draftIdempotencyKeyRef.current,
        operation: isNew ? "create" : "update",
        productId: isNew ? null : (id as string),
        expectedUpdatedAt:
          !isNew && typeof form.updated_at === "string" && form.updated_at
            ? form.updated_at
            : null,
        expectedAggregateRevision:
          !isNew && Number.isSafeInteger(Number(form.aggregate_revision))
            ? Number(form.aggregate_revision)
            : null,
        product: productRow,
        sourceForm: safePayload,
      });

      if (aggregateSave.ok === false) {
        setSubmitError(aggregateSave.message);
        toast.error(aggregateSave.message);
        return;
      }

      const savedId = aggregateSave.value.product_id;
      if (aggregateSave.value.pricing_rules_written > 0) {
        await loadChannelAuthority(savedId);
      }

      const reloaded = dbProductToForm(aggregateSave.value.product);
      ...
      setDirty(false);
      setForm(reloaded);
      // A fresh logical edit must never reuse the successful request identity with a
      // different payload. Retries before success retain the old key via the draft.
      draftIdempotencyKeyRef.current = createProductDraftIdempotencyKey();
      toast.success("Saved");
      if (isNew) {
  ...
  return (
    <>
      ...
      {aggregateBlockingIssues.length > 0 && (
        <div className="mb-4 rounded-lg border border-destructive/30 bg-destructive/5 p-3 text-sm">
          <div className="font-semibold text-destructive">Data-integrity blockers</div>
          <div className="mt-2 flex flex-wrap gap-2">
            {aggregateBlockingIssues.map((issue) => (
              <Button
      ...
      <div className="grid lg:grid-cols-3 gap-6 min-w-0">
        <div className="lg:col-span-2 space-y-6 min-w-0">
          ...
          >
            <div className="sticky top-0 z-20 -mx-3 mb-4 border-b border-border/60 bg-background/95 px-3 py-2 backdrop-blur sm:mx-0 sm:px-0">
              <div className="flex items-center gap-3 sm:hidden">
                <div className="min-w-0 flex-1">
                  <Label htmlFor="product-editor-section" className="sr-only">Editor section</Label>
                  <select
                    id="product-editor-section"
                    className="flex h-11 w-full rounded-md border border-input bg-background px-3 py-2 text-sm font-medium"
                    value={tab}
                    onChange={(event) => startTransition(() => setTab(event.target.value))}
                  >
                    {availableEditorTabs.map((item) => (
                      <option key={item.value} value={item.value}>{item.label}</option>
                    ))}
                  </select>
                </div>
                <div className="shrink-0 text-xs text-muted-foreground" aria-live="polite">
                  {Math.max(1, availableEditorTabs.findIndex((item) => item.value === tab) + 1)} / {availableEditorTabs.length}
                </div>
              </div>
              <div className="hidden overflow-x-auto sm:block">
              <TabsList className="flex h-auto w-max min-w-full justify-start gap-1 bg-transparent px-0 py-0">
            ...
            <TabsContent value="uom" className="space-y-6">
              <div className="card-elevated p-6 space-y-5">
                ...
                <div className="grid sm:grid-cols-3 gap-4">
                  ...
                  <Field label="Pieces per kg" field="pieces_per_kg">
                    <Input type="number" value={form.pieces_per_kg ?? ""} onChange={(e) => set("pieces_per_kg", e.target.value)} placeholder="55" />
                  </Field>
                  <Field label="Approx. piece weight (g)" field="approximate_piece_weight_g">
              ...
              <div className="card-elevated p-6 space-y-5">
                ...
                <div className="grid sm:grid-cols-4 gap-4">
                  <Field label="Primary Pack Type" field="primary_pack_type">
                    <Select
                  ...
                  <Field label="Pack UOM" field="primary_pack_uom">
                    <Select
                  ...
                  <Field label="Qty per Pack" field="qty_per_pack">
                    <Input
                      type="number"
                  ...
                  <Field label="Qty Content UOM" field="qty_content_uom">
                    <Select
                ...
                <div className="card-elevated p-6 space-y-4">
                  ...
                  {form.private_label_allowed && (
                    <div className="grid sm:grid-cols-2 gap-4">
                      <Field label="Private label MOQ" field="private_label_moq">
                      ...
                      <Field label="Cost per unit (₹)" field="private_label_cost_per_unit">
                      ...
                      <Field label="Upfront cost (₹)" field="private_label_upfront_cost">
            ...
            {showFrozen && (
              <TabsContent value="frozen" className="space-y-6">
                <div className="card-elevated p-6 space-y-4">
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <Field label="Frozen shelf life (days)" field="frozen_shelf_life_days">
                      <Input type="number" value={form.frozen_shelf_life_days ?? ""} onChange={(e) => set("frozen_shelf_life_days", e.target.value)} placeholder="180" />
                    </Field>
                    <Field label="Post-processing shelf life (days)" field="post_processing_shelf_life_days">
            ...
            {!isNew && (
              <TabsContent value="channels" className="space-y-6">
                <ChannelMoqRules
                  ...
                  product={form}
                  onRulesChange={() => {
                    if (id) void loadChannelAuthority(id).then(() => refreshAggregateRevision(id));
                  }}
                />
                <ChannelPricingRules
                  productId={id!}
            ...
            <TabsContent value="compliance" className="space-y-6">
              ...
              <div className="card-elevated p-6">
                ...
                <div className="grid sm:grid-cols-3 gap-4">
                  ...
                  <Field label="GST %" field="gst_rate">
