...
import { type FeatureFlag, refreshFeatureFlags, useFeatureFlags } from "@/hooks/useFeatureFlags";
  ...
  return (
    <div className="card-elevated p-4 mb-6 border-warning/40">
      <div className="flex items-center gap-2 mb-2">
        ...
        <h3 className="font-display">Clear imported PDF page hero images</h3>
      </div>
      <p className="text-sm text-muted-foreground mb-2">
        {count === null ? "Counting…" : `${count} products use a PDF screenshot as hero.`}
      </p>
  ...
  return (
    <div className="card-elevated p-4 space-y-3">
      <div className="flex items-start justify-between gap-2">
        ...
        <span
          className={`badge-soft text-[10px] uppercase ${STATUS_TONE[f.status] ?? "bg-muted text-muted-foreground"}`}
        >
          {f.status}
        </span>
      </div>
      <div className="flex flex-wrap gap-1 text-[11px] text-muted-foreground">
        ...
        {integ && (
          <>
            <span>·</span>
            <span>Secret: {integ.secret_status}</span>
          </>
        )}
        {f.last_tested_at && (
          <>
            <span>·</span>
            <span>Last test: {new Date(f.last_tested_at).toLocaleString()}</span>
          </>
        )}
      </div>
      ...
      >
        {open ? <ChevronDown className="h-3 w-3" /> : <ChevronRight className="h-3 w-3" />} Setup
        checklist
      </button>
      {open && (
      ...
      {f.last_test_result && (
        <p className="text-[11px] text-muted-foreground border-t pt-2">
          Last result: {f.last_test_result}
        </p>
      )}
      ...
      {canManage ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 pt-1">
          ...
          {f.is_visible ? (
            <Button size="sm" variant="outline" disabled={!!busy} onClick={hide}>
              <EyeOff className="h-3 w-3 mr-1" />
              Hide
            </Button>
          ) : (
            <Button size="sm" variant="outline" disabled={!!busy} onClick={show}>
...
const Settings = () => {
  ...
  const loadIntegs = useCallback(async () => {
    if (!PRODUCTION_CAPABILITIES.integrationSettings) {
      setIntegs([]);
      return;
    }
  ...
  const integByKey = useMemo(
    () => Object.fromEntries(integs.map((i) => [i.integration_key, i])),
    [integs],
  );

  const ready = flags.filter((f) => f.is_enabled || f.status === "test_passed");
  const readySet = new Set(ready.map((f) => f.feature_key));
  const setup = flags.filter(
    (f) =>
      !readySet.has(f.feature_key) &&
      (f.status === "configured" || f.status === "planned" || f.status === "error"),
  );
  ...
  return (
    <>
      <PageHeader
        title="Future Features & Integration Activation"
        subtitle="Enable, hide, and test future-ready modules. Secrets stay in backend."
      />
      <PdfHeroCleanup />
      ...
      <div className="card-elevated p-4 mb-6 bg-accent-soft/40 text-sm flex items-start gap-2">
        ...
        <span>
          API secrets are <strong>never stored in the frontend</strong>. Add them as backend (Edge
          Function) secrets. SKU and product_id are permanent identities — alias names are search
          helpers only and must never be used as external references.
        </span>
      </div>
      ...
      {!canManage && (
        <div className="card-elevated p-4 mb-6 text-sm flex items-start gap-2">
          ...
          <span>You can view feature status. Only Owner/Admin can change activation.</span>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="space-y-8">
          <Section
            ...
            tone="success"
            items={ready}
            integByKey={integByKey}
            canManage={canManage}
            onChange={refreshAll}
            empty="No features enabled yet."
          />
          <Section
            ...
            canManage={canManage}
            onChange={refreshAll}
            empty="No features awaiting setup."
          />
          <Section
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {items.map((f: FeatureFlag) => (
          <FeatureCard
            key={f.feature_key}
            f={f}
            integ={integByKey[f.feature_key]}
            canManage={canManage}
            onChange={onChange}
          />
        ))}
