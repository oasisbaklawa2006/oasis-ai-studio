...
export function parseStoredResolution(
  ...
  const r = value as ProductUtteranceResolution;
  if (typeof r.query !== "string" || typeof r.confidence_band !== "string") return null;
  return r;
}
...
export function rowFromRpcPayload(data: Record<string, unknown>): WhatsAppInboundMessageRow {
  return {
    ...
    resolver_result_json: parseStoredResolution(data.resolver_result_json),
