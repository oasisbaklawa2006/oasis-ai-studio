...
function buildIdentity(p: ReadinessProductInput): ReadinessCategory {
  ...
  if (!hasText(p.description) && !hasText(p.short_description)) {
    return {
      ...
      label: "Identity",
      state: "warn",
      detail: "Name is set, but no description is set.",
  ...
  return {
    ...
    label: "Identity",
    state: "pass",
    detail: "Name and description are set.",
    nextAction: null,
    group: "general",
  };
}
...
function buildSku(p: ReadinessProductInput): ReadinessCategory {
  if (!hasText(p.sku)) {
    return {
      ...
      nextAction: "Generate or set a SKU in Product Catalogue.",
      group: "general",
    };
  }
  return {
...
function buildCategory(p: ReadinessProductInput): ReadinessCategory {
  ...
  if (!hasText(p.subcategory)) {
    return {
      ...
      label: "Category",
      state: "warn",
      detail: `Category is "${p.category}", but Subcategory is not set.`,
      nextAction: "Set Subcategory for finer catalogue navigation.",
      group: "general",
    };
  }
  return {
    ...
    label: "Category",
    state: "pass",
    detail: `Category: ${p.category} · ${p.subcategory}`,
    nextAction: null,
    group: "general",
  };
}
...
function buildHeroImage(p: ReadinessProductInput): ReadinessCategory {
  if (!hasText(p.hero_image_url)) {
    return {
      ...
      label: "Hero Image",
      state: "missing",
      detail: "No hero image set.",
      nextAction: "Add a hero image in Media Library.",
      group: "general",
    };
  }
  return {
...
function buildPricing(p: ReadinessProductInput): ReadinessCategory {
  ...
  if (!hasMrp && !hasB2b) {
    return {
      ...
      label: "Pricing",
      state: "missing",
      detail: "No MRP or B2B price set.",
      nextAction: "Set at least one price so catalogue/B2B copy can reference it.",
      group: "general",
    };
  }
  if (!hasMrp || !hasB2b) {
    return {
      ...
      label: "Pricing",
      state: "warn",
      detail: !hasMrp ? "B2B price is set, MRP is blank." : "MRP is set, B2B price is blank.",
      nextAction: !hasMrp ? "Set MRP for retail listings." : "Set B2B price for wholesale copy.",
      group: "general",
    };
  }
  return {
    ...
    label: "Pricing",
    state: "pass",
    detail: `MRP ₹${p.mrp} · B2B ₹${p.b2b_price}`,
    nextAction: null,
    group: "general",
  };
}
...
function buildCatalogueVisibility(p: ReadinessProductInput): ReadinessCategory {
  ...
  if (!isActive) {
    return {
      ...
      nextAction: "Activate the product if this SKU should be usable again.",
      group: "general",
    };
  }
  if (!isReady) {
    return {
      ...
      label: "Catalogue Visibility",
      state: "warn",
      detail: "Active, but not yet marked catalogue-ready.",
      nextAction: "Confirm catalogue readiness in Product Catalogue once content is approved.",
      group: "general",
    };
  }
  return {
    ...
    label: "Catalogue Visibility",
    state: "pass",
    detail: "Active and catalogue-ready.",
    nextAction: null,
    group: "general",
  };
}
...
function buildPackSize(p: ReadinessProductInput): ReadinessCategory {
  ...
  if (!hasPack || !hasWeight) {
    return {
      ...
      state: "warn",
      detail: !hasPack
        ? "Net weight is set, Pack Size is blank."
        : "Pack Size is set, Net weight is blank.",
      nextAction: !hasPack ? "Set Pack Size." : "Set Net Weight (g).",
      group: "packaging",
    };
  }
  return {
    ...
    label: "Pack Size",
    state: "pass",
    detail: `${p.pack_size} · ${p.net_weight_g}g net`,
    nextAction: null,
    group: "packaging",
  };
}
...
function buildCartonPackaging(p: ReadinessProductInput): ReadinessCategory {
  const hasCartonQty =
    hasNumber(p.pcs_per_carton) || hasNumber(p.carton_qty) || hasNumber(p.master_carton_qty);
  const hasDims =
  ...
  if (!hasCartonQty && !hasDims) {
    return {
      key: "carton_packaging",
      label: "Carton / Master Packaging",
      state: "missing",
      detail: "No carton quantity or carton dimensions set.",
      nextAction: "Fill in carton quantity and carton dimensions.",
      group: "packaging",
    };
  }
  if (!hasCartonQty || !hasDims) {
    return {
      ...
      state: "warn",
      detail: !hasCartonQty
        ? "Carton dimensions are set, carton quantity is blank."
        : "Carton quantity is set, carton dimensions are blank.",
      nextAction: !hasCartonQty
        ? "Set pieces/carton or master carton qty."
        : "Set carton dimensions (cm).",
      group: "packaging",
    };
  }
  return {
...
function buildShelfStorage(p: ReadinessProductInput): ReadinessCategory {
  ...
  if (!hasShelf || !hasStorage) {
    return {
      ...
      state: "warn",
      detail: !hasShelf
        ? "Storage is set, Shelf Life (days) is blank."
        : "Shelf life is set, Storage Instructions is blank.",
...
function buildExportCompliance(p: ReadinessProductInput): ReadinessCategory {
  ...
  if (!hasHsn && !hasGst) {
    return {
      ...
      label: "HSN / GST",
      state: "missing",
      detail: "No HSN code or GST rate set.",
      nextAction: "Set HSN Code and GST Rate for export/compliance copy.",
      group: "packaging",
    };
  }
  if (!hasHsn || !hasGst) {
    return {
      ...
      state: "warn",
      detail: !hasHsn
        ? "GST rate is set, HSN Code is blank."
        : "HSN Code is set, GST rate is blank.",
      nextAction: !hasHsn ? "Set HSN Code." : "Set GST Rate.",
      group: "packaging",
    };
  }
  return {
    ...
    label: "HSN / GST",
    state: "pass",
    detail: `HSN ${p.hsn_code} · GST ${p.gst_rate}%`,
    nextAction: null,
    group: "packaging",
  };
}
