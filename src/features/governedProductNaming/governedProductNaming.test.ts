...
describe("detectUnsafeNamingClaims", () => {
  ...
  it("allows superlatives that are part of authoritative identity", () => {
  ...
  it("flags medical and nutritional claims", () => {
    expect(detectUnsafeNamingClaims("Treats diabetes naturally.", BASE_FACTS).length).toBeGreaterThan(0);
    expect(detectUnsafeNamingClaims("Sugar-free indulgence.", BASE_FACTS).length).toBeGreaterThan(0);
  });
...
describe("buildHeuristicNamingSuggestions", () => {
  ...
  it("never emits legacy Premium/signature filler", () => {
    const result = buildHeuristicNamingSuggestions(BASE_FACTS);
    expect(result.ok).toBe(true);
    if (!result.ok) return;
...
describe("validateGovernedCatalogueCopy", () => {
  ...
  it("accepts fact-grounded provider output", () => {
    expect(validateGovernedCatalogueCopy(validContent, BASE_FACTS).ok).toBe(true);
  });

  it("rejects unsafe superlative output", () => {
    const result = validateGovernedCatalogueCopy(
      { ...validContent, long_description: "The best baklawa you will ever taste." },
      BASE_FACTS,
    );
    expect(result.ok).toBe(false);
    if (result.ok) return;
  ...
  it("rejects empty fields", () => {
    expect(validateGovernedCatalogueCopy({ ...validContent, short_description: "" }, BASE_FACTS).ok).toBe(false);
  });
});

describe("validateProviderReviewEnvelope", () => {
  it("requires human-review and review-only markers", () => {
    expect(validateProviderReviewEnvelope({ ok: true, human_review_required: true, suggestion_only: true, approved: false }).ok).toBe(true);
    expect(validateProviderReviewEnvelope({ ok: true, human_review_required: false }).ok).toBe(false);
    expect(validateProviderReviewEnvelope({ ok: true, human_review_required: true, approved: true }).ok).toBe(false);
  });
});
...
describe("mockCatalogueAiCopyProvider", () => {
  ...
  it("fails closed on review-marker, unsafe-copy, and schema faults", () => {
    expect(mockCatalogueAiCopyProvider(BASE_FACTS, "missing_review_marker").parseResult.ok).toBe(false);
    expect(mockCatalogueAiCopyProvider(BASE_FACTS, "unsafe_superlative").parseResult.ok).toBe(false);
    expect(mockCatalogueAiCopyProvider(BASE_FACTS, "invalid_schema").parseResult.ok).toBe(false);
  });
});
...
describe("validateNamingText", () => {
  ...
  it("rejects invented ingredient claims without authoritative description", () => {
    expect(validateNamingText("Crafted with premium pistachio and honey.", { product_name: "Test Product" }).ok).toBe(false);
  });
});
