...
describe("Core knowledge bridge RPC contract", () => {
  ...
  it("matches ExtendedDatabase RPC arg contract from merged Core migration", () => {
    ...
    expect(keys).toEqual([...EXPECTED_CORE_RPC_ARGS].sort());
    expect(shape.p_schema_version).toBe("wa-knowledge/v1");
    expect(shape.p_handoff_eligibility).toBe("HANDOFF_READY");
  });
});
