...
export function validateProviderChannelEnvelope(
  ...
  if (typeof row.source_version !== "string" || !row.source_version.trim()) {
    return { ok: false, reason: "Channel-copy response is missing source_version." };
  }
  if (typeof row.channel_prompt_version !== "string" || !row.channel_prompt_version.trim()) {
    return { ok: false, reason: "Channel-copy response is missing channel_prompt_version." };
  }
  return { ok: true };
}
