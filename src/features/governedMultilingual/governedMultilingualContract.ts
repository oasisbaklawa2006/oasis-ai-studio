...
function factsBlob(source: AuthoritativeMultilingualSource): string {
...
export function isSupportedMultilingualLocale(locale: string): locale is MultilingualLocaleCode {
...
export function validateMultilingualText(
  ...
  return {
    ok: true,
    suggestion: suggestion(source, { kind, locale, value: text, availability: "available" }),
  };
}
...
function aliasSeedsToSuggestions(
  ...
  for (const seed of seeds) {
    ...
    out.push({
      ...check.suggestion,
      kind,
      script: seed.script || MULTILINGUAL_LOCALE_SCRIPTS[locale],
...
export function buildHeuristicMultilingualSuggestions(
  ...
  for (const row of suggestions) unique.set(`${row.kind}::${row.locale}::${row.value}`, row);
  return {
    ok: true,
    suggestion_only: true,
    approved: false,
