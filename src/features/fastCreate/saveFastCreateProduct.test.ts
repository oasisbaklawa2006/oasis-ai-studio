...
describe("requireFastCreateSku — packaging authority", () => {
  ...
  it("regenerates when the existing SKU packaging segment is stale", async () => {
...
describe("buildFastCreateGroupedDraftPayload", () => {
  ...
  it("preserves factual-composition and live legal fields in the governed compliance payload", () => {
    const payload = buildFastCreateGroupedDraftPayload(
      {
        product_name: "Test Product",
        category: "Baklawa",
        product_class: "bulk_loose_product",
        ingredients: "Filo, cashew",
        allergen_warnings: "Contains nuts and gluten",
        storage_instructions: "Store cool and dry",
        shelf_life_days: 90,
        fssai_licence_number: "10000000000000",
        country_of_origin: "India",
        label_manufacturer_details: "Oasis Baklawa, New Delhi",
      },
      null,
      minimalSuggestions,
      {
    ...
    ) as { compliance?: Record<string, unknown> };
    expect(payload.compliance?.ingredients).toBe("Filo, cashew");
    expect(payload.compliance?.country_of_origin).toBe("India");
  });
});
...
describe("saveFastCreateProduct — canonical governed draft only", () => {
  ...
  it("submits catalogue_product_drafts via Core RPC for privileged roles and never inserts products directly", async () => {
...
describe("saveFastCreateProduct — internal sale type never becomes sellable", () => {
  beforeEach(() => {
    rpcMock.mockClear();
  });
  ...
  it("blocks internal_bom even when category defaults already set product_class", async () => {
    await expect(
      saveFastCreateProduct({
        suggestions: {
          ...minimalSuggestions,
  ...
  it("allows b2b_horeca with structured SKU in draft payload", async () => {
    const result = await saveFastCreateProduct({
      suggestions: minimalSuggestions,
      heroUrl: null,
      roles: ["owner"],
