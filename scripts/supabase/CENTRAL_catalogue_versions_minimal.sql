-- Minimal Central deploy for AI Studio Central Sync preview persistence.
-- Apply manually in Supabase SQL Editor on tcxvcatsqqertcnycuop when ready.
-- Requires: public.products, public.is_team_member(uuid) (Central role map).
...
DROP POLICY IF EXISTS catalogue_versions_team_rw ON public.catalogue_versions;
CREATE POLICY catalogue_versions_team_rw ON public.catalogue_versions
  FOR ALL TO authenticated
  USING (public.is_team_member(auth.uid()))
  WITH CHECK (public.is_team_member(auth.uid()));

DROP POLICY IF EXISTS catalogue_sync_events_team_rw ON public.catalogue_sync_events;
CREATE POLICY catalogue_sync_events_team_rw ON public.catalogue_sync_events
  FOR ALL TO authenticated
  USING (public.is_team_member(auth.uid()))
  WITH CHECK (public.is_team_member(auth.uid()));

GRANT SELECT, INSERT, UPDATE, DELETE ON public.catalogue_versions TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.catalogue_sync_events TO authenticated;
